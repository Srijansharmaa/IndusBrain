...
export default function HeroSection({ user, onAskCopilot }) {
  ...
  return (
    <Card hover={false} className="bg-gradient-to-br from-navy to-[#17233B] border-none mb-5">
      <div className="flex justify-between flex-wrap gap-4">
        <div className="max-w-[560px]">
          <Badge tone="primary" className="!text-blue-300 !bg-navy-soft">{role?.label}</Badge>
          <h1 className="text-white text-2xl font-extrabold mt-3 mb-1.5 tracking-tight">
        ...
        <div className="grid grid-cols-2 gap-2.5 min-w-[220px]">
          {HERO_STATS.map((stat) => (
            <div key={stat.label} className="bg-[#0F172A] border border-navy-line rounded-xl p-3">
